...
sudo docker pull lscr.io/linuxserver/plex:latest || error "Failed to pull the latest Plex docker image!"
...
docker run -d \
  ...
  lscr.io/linuxserver/plex:latest || error "Failed to run Plex docker image!"